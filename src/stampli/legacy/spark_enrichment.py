...
SRC_DIR = SCRIPT_PATH.parents[4] # enrichment->stampli->apps->biocontext->src
sys.path.append(str(SRC_DIR))

from stampli.paths import REVIEWS_BASE_PARQUET, REVIEWS_ENRICHED_V1
...
INGESTION_BATCH = 42000  # Full Run (was 100)
...
# 6. Run Enrichment (Iterative Batching to prevent "Hang")
if todo_count > 0:
    ...
    # 6.1 Configuration
    CHUNK_SIZE = 100  # Process 100 IDs per "Spark Action" (Safer saves, more feedback)
    # Dynamic Parallelism
    NUM_PARTITIONS = 100 if LLM_PROVIDER == "openai" else 2
    if LLM_PROVIDER == "ollama":
        CHUNK_SIZE = 50 # Smaller chunks for local
    
    print(f"Starting Processing loop. Total To Do: {todo_count}")
    print(f"Partitions: {NUM_PARTITIONS} | Batch Size: {CHUNK_SIZE}")

    # 6.2 Get List of IDs to process (Strings)
    # This is small enough to fit in Driver RAM (42k uids = 2MB)
    ...
    for i in range(total_chunks):
        ...
        # 6.3.1 Filter Base Data for this Chunk
        # Using IsIn is efficient for 500 items
        df_chunk = sdf_base.filter(col("review_uid").isin(current_chunk_uids))
        
        # 6.3.2 Repartition & Enrich
        # For small chunks (500), 100 partitions -> 5 rows per task. Very fast.
        # But for OpenAI, we want high concurrency.
        ...
        # 6.3.4 Write (Append) - This triggers the computation
        # Try/Except to catch interrupts gracefully?
        try:
            final_sdf.write.mode("append").parquet(ENRICHED_PATH)
            print(">> Saved Chunk.")
        except Exception as e:
            print(f"Error saving chunk {i+1}: {e}")
            raise e # Stop script if write fails
